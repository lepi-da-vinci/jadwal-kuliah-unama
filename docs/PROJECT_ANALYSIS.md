...
| **2026-09-07** | **Pemisah Database Semester Perkuliahan (Multi-Semester)** | Penambahan fitur pemisahan dataset per semester akademik (e.g. `Genap 2025`, `Ganjil 2026`) untuk mencegah jadwal semester sebelumnya terhapus saat portal BAAK berganti semester. Dilengkapi tabel master `semester`, kolom `semester` pada `jadwal`/`jadwal_temp`/`notifikasi_lab`, auto-detect nama semester dari teks header portal BAAK, endpoint `/api/semesters`, switcher badge interaktif di header, dan sub-view manajemen database semester di Modal Setting. |
| **2026-09-22** | **Pusat Statistik & Analitik Akademik (Dedicated Explorer & Scoped Chatbot)** | Implementasi fitur analitik penggunaan laboratorium, kelas, dan dosen: (1) Tombol navigasi di Modal Setting tepat di bawah Mode Layar TV, (2) Dedicated Fullscreen View (`stats-explorer.html`) dengan Semester Switcher, 4 KPI hero cards, dan 3 tab analitik interaktif (Lab, Kelas, Dosen), (3) Backend endpoint `@app.get("/api/statistics")` dengan agregasi jam terbang dan kalkulasi utilisasi akurat, (4) WhatsApp Chatbot tools (`get_statistik_lab_saya` & `get_statistik_akademik`) dengan strict scoping (default hanya lab aslab sendiri, 0 emoji, dan kepatuhan single source of truth BAAK). |
