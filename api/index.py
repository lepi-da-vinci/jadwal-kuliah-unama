...
@app.get("/api/cron/notifikasi")
async def cron_notifikasi():
    """Endpoint untuk dipanggil oleh Vercel Cron Job setiap 1 menit"""
    try:
        query = '''
            SELECT 
                j.hari, 
...
async def cron_notifikasi():
    """Endpoint untuk dipanggil oleh Vercel Cron Job setiap 1 menit"""
    try:
        await wa_notifier.run_wa_notifier_check()
        return {"status": "success", "message": "Pengecekan notifikasi WA selesai"}
    except Exception as e:
        return {"status": "error", "message": str(e)}
